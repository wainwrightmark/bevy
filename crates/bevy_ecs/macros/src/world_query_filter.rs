...
#[derive(Default)]
struct WorldQueryFilterAttributes {
...
pub static WORLD_QUERY_FILTER_ATTRIBUTE_NAME: &str = "world_query_filter";
...
pub fn derive_world_query_filter_impl(input: TokenStream) -> TokenStream {
    ...
    let mut fetch_struct_attributes = WorldQueryFilterAttributes::default();
    for attr in &ast.attrs {
        if !attr
            .path()
            .get_ident()
            .map_or(false, |ident| ident == WORLD_QUERY_FILTER_ATTRIBUTE_NAME)
        {
            continue;
        }
        ...
        attr.parse_args_with(|input: ParseStream| {
            ...
            for meta in meta {
                ...
                if ident == DERIVE_ATTRIBUTE_NAME {
        ...
        .unwrap_or_else(|_| panic!("Invalid `{WORLD_QUERY_FILTER_ATTRIBUTE_NAME}` attribute format"));
    }
    ...
    let (mutable_struct, mutable_impl) = impl_fetch;
